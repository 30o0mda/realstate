...
class DataStatus
{
    ...
    */
    ...
    public function DesktopResponse(string $dataKeyName = 'data')
    {
        $response = [];
        ...
        $response['success'] = $this->getStatus();
        if($this->getStatus() !== JsonResponse::HTTP_OK && !empty($this->getErrors())) {
            $response['errors'] = $this->getErrors();
        }
        $response['message'] = $this->getMessage();
        if($this->getStatusTitle() === self::STATUS_SUCCESS && $this->getStatus()) {
            $response[$dataKeyName] = $this->getResourceData() ?? $this->getData();
            if($response[$dataKeyName] === null) {
                unset($response[$dataKeyName]);
            }
        }
