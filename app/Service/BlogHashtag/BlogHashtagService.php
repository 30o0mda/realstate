...
class BlogHashtagService
{
    ...
    public function fetchBlogHashtag($data):DataStatus
    {
        ...
        if (isset($data['with_pagination']) && $data['with_pagination'] == 1) {
            ...
            $response = BlogHashtagResource::collection($all_blog_hashtag)->response()->getData(true);
        } else {
    ...
    public function fetchBlogHashtagDetails($data):DataStatus
    {
        $blogHashtag = BlogHashtag::find($data['blog_hashtag_id']);
        return DataSuccess::make(resourceData:new BlogHashtagResource($blogHashtag), message:'Blog hashtag details fetched successfully');
    }
